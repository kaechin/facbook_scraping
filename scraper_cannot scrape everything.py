...
class Facebook_scraper:
    ...
    def __init__(self, page_name, browser="chrome", proxy=None, timeout=600, headless=True):
    ...
    def scrap_to_json(self, start_date=None, end_date=None):
        ...
        while True:
            print("Hello from me")
            last_post_date = self.__scroll_to_bottom(start_date, end_date)

            # Check if the last loaded post is older than the start_date
            if end_date and last_post_date and last_post_date < end_date:
                break

            # Check if the number of loaded posts is greater than or equal to the desired posts_count
            print(len(self.__data_dict))
            #if len(self.__data_dict) >= self.posts_count:
            #    break
    ...
    def __scroll_to_bottom(self, start_date=None, end_date=None):
        last_post_date = None
        while True:
            ...
            for post in all_posts:
                try:
                    ...
                    loves = Scraping_utilities._Scraping_utilities__find_reaction_by_text(l, "Love")
                    wow = Scraping_utilities._Scraping_utilities__find_reaction_by_text(l, "Wow")
                    ...
                    haha = Scraping_utilities._Scraping_utilities__find_reaction_by_text(l, "Haha")
                    ...
                    reactions = {"likes": int(likes), "loves": int(loves), "wow": int(wow), "cares": int(cares),
                                 "sad": int(sad), "angry": int(angry), "haha": int(haha)}
                    ...
                    # Update the last_post_date with the current post's date
                    posted_date = datetime.strptime(posted_time.split("T")[0], "%Y-%m-%d")
                    if last_post_date is None or posted_date > last_post_date:
                        last_post_date = posted_date
                    ...
                    logger.exception("Error at scroll_to_bottom method : {}".format(ex))

            # Call __find_elements to process the found posts
            self.__find_elements(self.page_name, start_date, end_date)
            ...
            # # Check if the last loaded post is older than the start_date
            # if start_date and last_post_date and last_post_date < start_date:
            #     break

            # Check if the last loaded post is older than the specified end_date
            if end_date and last_post_date and last_post_date < end_date:
                break
    ...
    def __find_elements(self, name, start_date=None, end_date=None):
        ...
        for post in all_posts:
            try:
                ...
                # Convert posted_time to a date object for comparison
                posted_date = datetime.strptime(posted_time.split("T")[0], "%Y-%m-%d") # Adjust the date format if needed

                if start_date and end_date and not (start_date <= posted_date <= end_date):
                    # if posted_date < end_date:
                    #     break
                    # else:
                        continue
